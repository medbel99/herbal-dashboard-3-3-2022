...
export default function useProduct() {
    return {
        thumbnailImage: (payload) => {
            if (payload) {
                if (payload.imageProduct) {
                    return (
                        <>
                            <LazyLoad>
                                <img
                                    src={`https://herbalbackend.herokuapp.com/api/upload/image/download/${payload.imageProduct.idBrowserPhoto}`}
                                    alt={payload.imageProduct.idBrowserPhoto}
                                />
                            </LazyLoad>
                        </>
                    );
        ...
        title: (payload) => {
            let view = (
                <Link href="/product/[pid]" as={`/product/${payload.idClientProducts}`}>
                    <a className="ps-product__title">{payload.titleArticle}</a>
                </Link>
            );
            return view;
        },
